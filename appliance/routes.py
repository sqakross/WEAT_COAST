...
def inventory_list():
    ...
    # Never allow URL filter to escape user's warehouse access.
    if (
    ...
    if status in valid_statuses:
        ...
        query = query.filter(
            ApplianceUnit.status == "available"
        )
    ...
    if condition in valid_conditions:
        query = query.filter(
            ApplianceUnit.condition == condition
        )
    else:
        condition = ""
    ...
    units = (
        ...
        .order_by(
            ApplianceCategory.sort_order.asc(),
            ApplianceCategory.name.asc(),
            ApplianceUnit.inventory_number.desc(),
        )
        .limit(500)
        .all()
    )
    ...
    for unit in units:
        if unit.category_id != current_category_id:
            current_category_id = unit.category_id
        ...
        current_group["units"].append(
            unit
        )

    # --------------------------------------------------------
    # Warehouses shown in filter must also respect access.
    # --------------------------------------------------------
    ...
    # Financial columns are shown only when user has pricing
    # permission for at least one warehouse currently visible.
    pricing_scope_ids = (
        [warehouse_id]
        if warehouse_id is not None
        else allowed_warehouse_ids
    )
