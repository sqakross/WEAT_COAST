...
#
# One row = one repair cycle for one physical appliance.
#
# Internal WCCR repair document:
#   REP-000001, REP-000002, ...
#
# Repair is performed by EXACTLY ONE of:
#
#   vendor   -> external vendor/company
#   internal -> specific internal User/Technician
#
# ApplianceMovement remains the immutable movement ledger.
# This table is the repair business document/state.
#
# One physical appliance may have multiple repair orders
# during its lifetime.
# ============================================================

class ApplianceRepairOrder(db.Model):
    __tablename__ = "appliance_repair_order"

    __table_args__ = (
        db.UniqueConstraint(
            "repair_number",
            name="uq_appliance_repair_order_number",
        ),
        db.Index(
            "ix_appliance_repair_order_unit_status",
            "appliance_unit_id",
            "status",
        ),
        db.Index(
            "ix_appliance_repair_order_warehouse_status",
            "warehouse_id",
            "status",
        ),
        db.Index(
            "ix_appliance_repair_order_type_status",
            "repair_type",
            "status",
        ),
        {"extend_existing": True},
    ...
    # Internal WCCR repair number.
    repair_number = db.Column(
        db.String(40),
        nullable=False,
        unique=True,
        index=True,
    ...
    #
    # vendor   = external company/vendor
    # internal = specific internal technician
    repair_type = db.Column(
        db.String(20),
        nullable=False,
        index=True,
    )

    # Used only when repair_type == "vendor".
    repair_vendor = db.Column(
        db.String(160),
        nullable=True,
        index=True,
    )

    # Used only when repair_type == "internal".
    #
    # This references a real WCCR User rather than storing
    # the technician name as free text.
    repair_technician_id = db.Column(
        db.Integer,
        db.ForeignKey(
            "user.id",
            ondelete="SET NULL",
    ...
    # Optional external vendor ticket / W/O / RMA.
    #
    # This is separate from our internal REP-xxxxxx number.
    ...
    # Final result of this repair cycle.
    #
    # Exact allowed outcomes will be controlled by the
    # Repair service when completion workflows are connected.
    outcome = db.Column(
        db.String(50),
        nullable=True,
        index=True,
    ...
    repair_technician = db.relationship(
        "User",
        foreign_keys=[repair_technician_id],
        lazy="select",
    )
    ...
    def completed_at_local(self):
        return utc_to_local(self.completed_at)
    ...
    def __repr__(self):
        return (
            f"<ApplianceRepairOrder id={self.id} "
            f"number={self.repair_number!r} "
            f"unit_id={self.appliance_unit_id} "
            f"type={self.repair_type!r} "
            f"status={self.status!r}>"
        )
