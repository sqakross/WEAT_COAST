...
from compare_cart.marcone_scraper import get_cart_items

def get_marcone_items():
...
def export_to_docx(results, filename="marcone_inventory_report.docx"):
    if os.path.exists(filename):
        os.remove(filename)

    document = Document()
    document.add_heading('Marcone Cart vs Inventory Report', 0)
